...
export class Utils {
  constructor(public ref: References, public logger: Logger) {}
  ...
  public addQuests(tables, imagerouter, modPath, modName, debugLogging): void {
    ...
    if (debugLogging) {
      this.logger.log(
        `Loaded ${imageCount} custom images and ${questCount} custom quests.`,
        LogTextColor.CYAN
      );
    }
  }
  ...
  public addQuests2(tables, imagerouter, modPath, modName, debugLogging): void {
    ...
    this.loadFiles(
      `${modPath}../db/questFiles2/locales/`,
      [".json"],
      (localepath) => {
        const Locales = require(localepath);
        for (const i in Locales) {
  ...
  public profileBackup(sessionID: string, profile: ISptProfile): void {
    ...
    const backupFile = `${
      backupPath + sessionID
    }_RO_${day}_${randomNum}-backup.json`;
    const maxBackups = 3;
  ...
  public getReqCost(itemID: string): number {
    return Math.ceil(
      this.getFleaPrice(itemID) / this.getFleaPrice("668b3c71042c73c6f9b00704")
    );
  }
  ...
  public buildPresetAssort(
    ...
    const slipCost = Math.round(
      presetPrice / this.getFleaPrice("668b3c71042c73c6f9b00704")
    );
    ...
    try {
      ...
      this.logger.log(
        `Error loading ${presetName} => ${error}, skipping preset.`,
        LogTextColor.RED
      );
    }
  }
...
export class AssortUtils {
  ...
  public getItemInHandbook(itemID: string, tables: any): number {
    try {
      const hbItem = tables.templates.handbook.Items.find(
        (i) => i.Id === itemID
      );
      return hbItem.Price;
    } catch (error) {
