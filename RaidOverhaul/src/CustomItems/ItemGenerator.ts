...
export class ItemGenerator {
  ...
  public createCustomItems(itemDirectory: string): void {
    ...
    for (const newId in this.itemConfig) {
      ...
      const tempClone =
        AllItemList[itemConfig["ItemToClone"]] || itemConfig["ItemToClone"];
  ...
  private createHandbook(
    itemConfig: CustomItemFormat[string],
    itemID: string
  ): IHandbookItem {
    const tables = this.ref.tables;
    // biome-ignore lint/complexity/useLiteralKeys: <explanation>
    ...
    if (itemConfig.Handbook !== undefined) {
      const tempHBParent =
        // biome-ignore lint/complexity/useLiteralKeys: <explanation>
        HandbookIDs[itemConfig["Handbook"]["HandbookParent"]] ||
        itemConfig["Handbook"]["HandbookParent"];
      ...
      const hbBase = tables.templates.handbook.Items.find(
        (i) => i.Id === itemToClone
      );
  ...
  private cloneToFilters(
    ...
    for (const item in tables.templates.items) {
      const itemConflictId =
        tables.templates.items[item]._props.ConflictingItems;
      ...
      for (const slots in tables.templates.items[item]._props.Slots) {
        const slotsId =
          tables.templates.items[item]._props.Slots[slots]._props.filters[0]
            .Filter;
      ...
      for (const cartridge in tables.templates.items[item]._props.Cartridges) {
        const cartridgeId =
          tables.templates.items[item]._props.Cartridges[cartridge]._props
            .filters[0].Filter;
      ...
      for (const chamber in tables.templates.items[item]._props.Chambers) {
        const chamberId =
          tables.templates.items[item]._props.Chambers[chamber]._props
            .filters[0].Filter;
  ...
  private addToBots(
    ...
    for (const botId in tables.bots.types) {
      ...
      for (const modItem in tables.bots.types[botId].inventory.mods) {
        for (const modSlot in tables.bots.types[botId].inventory.mods[
          modItem
        ]) {
          if (
            tables.bots.types[botId]?.inventory?.mods[modItem][modSlot][
              itemToClone
            ]
          ) {
            tables.bots.types[botId].inventory.mods[modItem][modSlot].push(
              itemID
            );
          }
        }
      ...
      itemConfig.CasePush?.CaseFiltersToAdd.forEach((caseToAdd) => {
        ...
        for (const item in items) {
          if (items[item]._id === cases) {
            if (
              items[item]._props?.Grids[0]._props.filters[0].Filter ===
              undefined
            ) {
              ...
              tables.templates.items[cases]._props.Grids[0]._props.filters =
                unbreakFilters;
            } else if (
              items[item]._props?.Grids[0]._props.filters[0].Filter !==
              undefined
  ...
  private pushToSlot(
    ...
    const tempSlot =
      SlotsIDs[itemConfig.SlotPush?.Slot] || itemConfig.SlotPush?.Slot;
  ...
  public createClothingTop(newTopConfig: any): void {
    const tables = this.ref.tables;
    const newTop = structuredClone(
      tables.templates.customization["5d28adcb86f77429242fc893"]
    );
    const newHands = structuredClone(
      tables.templates.customization[newTopConfig.HandsToClone]
    );
    const newSet = structuredClone(
      tables.templates.customization["5d1f623e86f7744bce0ef705"]
    );
    ...
    if (newTopConfig.TraderScheme !== undefined) {
      if (
        !tables.traders[newTopConfig.TraderScheme?.TraderToUse].base
          .customization_seller
      ) {
        tables.traders[
          newTopConfig.TraderScheme?.TraderToUse
  ...
  public createClothingBottom(newBottomConfig: any): void {
    const tables = this.ref.tables;
    const newBottom = structuredClone(
      tables.templates.customization["5d5e7f4986f7746956659f8a"]
    );
    const newSet = structuredClone(
      tables.templates.customization["5cd946231388ce000d572fe3"]
    );
    ...
    for (const locale in tables.locales.global) {
      tables.locales.global[locale][newBottomConfig.NewBottomsID] =
        newBottomConfig.LocaleName;
    }

    if (newBottomConfig.TraderScheme !== undefined) {
      if (
        !tables.traders[newBottomConfig.TraderScheme?.TraderToUse].base
          .customization_seller
      ) {
        tables.traders[
          newBottomConfig.TraderScheme?.TraderToUse
        ].base.customization_seller = true;
      }
